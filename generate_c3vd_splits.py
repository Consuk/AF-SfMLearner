...
def parse_frame_index_from_name(name):
    ...
    # Prefer full-numeric stems (e.g., 000123).
    if stem.isdigit():
        return int(stem)

    # Fallback: last number group in the stem.
...
def discover_sequences_under_subdir(data_path, split_subdir, exts):
    ...
    for seq_rel, frames in seq_to_frames.items():
        # Rebase from split subdir root back to full data root.
        if seq_rel == ".":
...
def discover_sequences_under_aliases(data_path, aliases, exts):
    """Try multiple subdir names and return the first non-empty map."""
    for name in aliases:
        seq_map = discover_sequences_under_subdir(data_path, name, exts)
        if seq_map:
            return seq_map, name

    # If no alias produced frames, still return the first existing directory map
    # (possibly empty), so callers can surface useful context.
    for name in aliases:
