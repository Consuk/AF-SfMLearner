...
from export_gt_depth import parse_split_line, find_c3vd_depth_file, load_depth, decode_c3vd_depth
...
def main():
    ...
    for i in range(n_check):
        ...
        folder, frame_token, _ = parse_split_line(lines[i])
        src_path = find_c3vd_depth_file(opt.data_path, folder, frame_token)
        if src_path is None:
            missing_source += 1
            print(f"[WARN] idx={i}: source depth file not found for '{lines[i]}'")
            continue

        src_depth = decode_c3vd_depth(load_depth(src_path)).astype(np.float32)
