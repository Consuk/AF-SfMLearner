...
class Trainer:
    ...
    def __init__(self, options):
        ...
        if self.opt.pose_model_type == "separate_resnet":
            ...
            self.models["pose"] = networks.PoseDecoder(
                self.models["encoder"].num_ch_enc, num_input_features=1,
                num_frames_to_predict_for=2)
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            pose_feats = self.models["pose_encoder"](torch.cat([inputs["color_aug", i, 0] for i in self.opt.frame_ids if i != 0], 1))

        if self.opt.pose_model_type in ["separate_resnet", "shared"]:
            axisangle, translation = self.models["pose"](pose_feats)
            outputs.update({"axisangle": axisangle, "translation": translation})
        else:
            outputs.update(self.models["pose"](pose_feats))

        # transform prediction
        transform_feats = self.models["transform_encoder"](
            torch.cat([inputs["color_aug", i, 0] for i in self.opt.frame_ids if i != 0], 1))
        axisangle_tr, translation_tr = self.models["transform"](transform_feats)
        outputs.update({"axisangle_tr": axisangle_tr, "translation_tr": translation_tr})

        # position prediction
        position_feats = self.models["position_encoder"](
            torch.cat([inputs["color_aug", i, 0] for i in self.opt.frame_ids if i != 0], 1))
        axisangle_pos, translation_pos = self.models["position"](position_feats)
